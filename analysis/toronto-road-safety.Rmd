...
The trend line for each city is fitted to the raw values using a simple least squared error method ^[ggplot2 includes convenient options for quickly fitting a line to a set of points].
...
The fitted city intercepts and slopes are available in the Appendix.
...
```{r nyc-toronto, fig.height=4, fig.align="center", fig.cap="Toronto and New York City pedestrian and cyclist fatality rates are visualized, along with 95-percent confidence intervals."}
nyc_toronto_plot
```
...
```{r amsterdam-toronto, fig.height=4, fig.align="center", fig.cap="Toronto and Amsterdam pedestrian and cyclist fatality rates are visualized, along with 95-percent confidence intervals."}
...
Based on our seven-year observation, only Copenhagen is safer than Amsterdam for pedestrians and cyclists ^[See Appendix for city scores.].
In fact, Toronto does not appear to be significantly more dangerous than Amsterdam to begin with ^[although this is partly a statistical artifact due in part to the unreliability of estimates at the time domain limits].
...
This paints a rather grim picture for the future of Toronto, because the trend is relatively steep in relation to the others and it is trending in the direction of greater danger.
...
If we calibrate our safety expectations based upon other cities, then this study presents evidence that Toronto compares unfavourably to the comparison cities that were studied.  Something about road safety is different in Toronto.  Although it is not possible to conclude *why* Toronto looks so different, there is at least a good reason to begin this investigation in earnest.  Therefore, there may well be reason to conclude that the pedestrian and cyclist fatality rate is actually high in the City of Toronto.
