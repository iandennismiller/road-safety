...
\begin{align*}
  fixed &= \textit{fatalities_per_capita} \sim 1 + \textit{year_offset} \\
  random &=~ \sim 1 + \textit{year_offset}~|~\textit{city}
\end{align*}

The trend lines were then extended into the future in order to predict pedestrian and cyclist fatality rates.  These results are plotted in Figure \@ref(fig:trends).

```{r trends, fig.height=4, fig.align="center", fig.cap="A "}
trends_plot
```

Estimations in the more distant future will be less reliable than those in the near future.  If these trends hold, then Toronto is on track to become more dangerous to pedestrians and cyclists than each other city.  By 2015, Toronto has already become indistinguishable from London, 
